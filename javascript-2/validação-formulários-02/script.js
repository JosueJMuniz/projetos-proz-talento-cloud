...
// ---------- VALIDAÇÃO Comfirma SENHA ---------- //
let confirmaInput = document.getElementById("confirma-senha");
let confirmaLabel = document.querySelector('label[for="confirma-senha"]');
let confirmaHelper = document.getElementById("confirma-senha-helper");


togglePopup(confirmaInput, confirmaLabel);
confirmaInput.addEventListener("blur", (e) => {
  ...
  if (valor == senhaInput.value) {
    ...
    confirmaHelper.innerText = "As senhas não coincidem."
...
/* Criando um objeto para checar os inputs válidos */
let inputsCorretos = {
  username: false,
  email: false,
  senha: false,
...
btnSubmit.addEventListener("click", (e) => {
  if (inputsCorretos.username == false || inputsCorretos.email == false ||
    ...
    e.preventDefault()
    alert("Os campos obrigatórios precisam se preenchidos corretamente!")
  } else {
